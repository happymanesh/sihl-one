...
import { istInstantFrom } from '@/lib/time';
 ...
 *
 * Both halves are read in the business timezone by the shared helper, so a
 * talk near midnight cannot end up on one day with the other day's time.
 */
function instantFrom(formData: FormData): string | undefined {
  return istInstantFrom(formData.get('date'), formData.get('time'));
}
